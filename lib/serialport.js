...
SerialPort.prototype.open = function(callback) {
  ...
  SerialPortBinding.open(this.path, this.options, function(err, fd) {
    ...
    if (process.platform !== 'win32') {
      this.serialPoller = new SerialPortBinding.SerialportPoller(this.fd);
      this.serialPoller.start(this.afterPoll.bind(this));
    }
    this.emit('open');
...
SerialPort.prototype.afterPoll = function(err) {
  if (err) {
    return this._disconnected(err);
  }
  this._read();
};
